...
private:
	...
	void toggle_item( bool state, int menu_item, int toolbar_item = -1 )
        {
        	if (menu_item >= 0)
                menu_partition_items[menu_item] ->set_sensitive( state ) ;
	...
	void allow_undo_clear_apply( bool state )
	{
		...
		static_cast<Gtk::CheckMenuItem *>( menu_main_items[MENU_UNDO_LAST_OPERATION] )
			->set_sensitive( state ) ; 

		static_cast<Gtk::CheckMenuItem *>( menu_main_items[MENU_CLEAR_OPERATIONS] )
			->set_sensitive( state ) ; 

		toggle_item( state, -1, TOOLBAR_APPLY ) ; 
		static_cast<Gtk::CheckMenuItem *>( menu_main_items[MENU_APPLY_OPERATIONS] )
			->set_sensitive( state ) ; 
	}
...
//gui stuff
    ...
    enum MenuMainItems
    {
    	MENU_DEVICES,
    	MENU_EDIT,
    	MENU_UNDO_LAST_OPERATION,
    	MENU_CLEAR_OPERATIONS,
    	MENU_APPLY_OPERATIONS,
    	MENU_VIEW,
    	MENU_VIEW_DEVICE_INFORMATIONS,
    	MENU_VIEW_OPERATIONS,
    	MENU_DEVICE,
    	MENU_PARTITION
    };
	enum MenuPartitionItems
    {
    	MENU_NEW,
    	MENU_DEL,
    	MENU_RESIZE_MOVE,
    ...
    std::map<int, Gtk::MenuItem*> menu_main_items;
    std::map<int, Gtk::MenuItem*> menu_partition_items;
